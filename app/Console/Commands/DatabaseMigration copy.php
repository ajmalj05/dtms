...
use App\Models\Masters\SubDiagnosisnMaster;
...
//                ->orderBy('id', 'ASC')
    ...
    private function _synchPatientTest()
    {
        ...
        for($i=0;$i<=125;$i++) //351 to 451 needed
        {
            if($i==0)
            {
                $skip=0;
...
foreach ($vital_ids as $vid) {
  ...
  //  if($vid==77) { if($vitalData->fibro_scan=="" || !$vitalData->fibr_scan) { $vital_value="";} else $vital_value= $vitalData->fibro_scan;}
...
private function _syncSubDiagnosisnMaster()
{
    $subDiagnosisMaster = DB::connection('mysqlSecondConnection')->table('subdiagnosis')->get();
    $this->info('Total Sub Diagnosis Master : ' . count($subDiagnosisMaster));
    
    $newDbSubDiagnosisMaster = [];
    
    foreach ($subDiagnosisMaster as $subDiagnosis) {
        $newDbSubDiagnosisMaster[] = [
            'id' => $subDiagnosis->id,
            'diagnosis_id' => $subDiagnosis->diagnosis_id,
            'code' => null,
            'branch_id' => $subDiagnosis->branch_id,
            'display_status' => 1,
            'is_deleted' => 0,
            'created_by' => 1,
            'created_at' => Carbon::now(),
            'updated_at' => Carbon::now(),
            'subdiagnosis_name' => $subDiagnosis->name
        ];
    }
    
    if (!empty($newDbSubDiagnosisMaster)) {
        // Assuming SubDiagnosis model is imported/defined
        SubDiagnosis::insert($newDbSubDiagnosisMaster);
    }
    
    $this->info('Total new Sub Diagnosis : ' . count($newDbSubDiagnosisMaster));
}
//
