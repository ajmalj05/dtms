...
// $this->_synch_alerts();
   $this->_synch_indivijual_test();

    }

    public function _synch_indivijual_test()
    {
    //    $tableName="anti_tg"; // id=1889
    //    $testId=1889;

    //    $tableName="gamma_test"; // id=1889  GAMMA
    //    $testId=43;


    // $tableName="cea"; // id=98  GAMMA
    // $testId=98;

    $tableName="vldl"; // 1534=98  VLDL
    $testId=1534;
       ...
       for ($i = 0; $i <= 167; $i++) {
        ...
        if($i>0){
            $skip=$i*$iterationLimit;
        }

        $test_results = DB::connection('mysqlSecondConnection')->table($tableName)
        ->select('*')
        // ->where('name','!=','')
        ->orderBy('id', 'ASC')
        ->skip($skip)
        ...
        $this->info('Total Test : ' . count($test_results));
        ...
        foreach ($test_results as $item)
            ...
            $this->info(' Total new test : ' . count($newData));
                ...
                foreach ($incs as $i) {
                    ...
                    if($time!= "" || $bps!="" || $bpd!="" || $pulse!="")
                    {
                       ...
                       // dd();
                        ...
                        $specialistId=getAfeild("id","patient_visits",$cond);
...
if(!$bggrp || $bggrp=="") $bggrp=0;
            ...
            $newDbPatientRegistration[] = [
               ...
               'claim_id' => $patient->claim,
                ...
                'blood_group_id' => $patient->newbrgp,
                'is_api_verify' => 0,
            ];
