...
class DtmsController extends Controller
{
    ...
    public function saveDiagnosisList(Request $request)
    {
        ...
        if ($validated) {
            ...
            if ($request->diag_check_bx == "on") {
                $is_question_mark = 1;
            } else {
                $is_question_mark = 0;
            }

            if ($request->newly_diag_check_bx == "on") {
                $newly_diagnosed = 1;
            } else {
                $newly_diagnosed = 0;
            }
     ...
     * @param Request $request
     */
    public function getPhotos1(Request $request)
    {
        $cond = [
    ...
    public function getDiagnosisData()
    {
        ...
        foreach ($patientDiagnosis as $item) {
            ...
            $diagnosisDate1 = Carbon::parse($item->$diagnosisYear)->format('d-m-Y');
            $diagnosisElements[] = [
                'id' => $item->id,
                'diagnosis_name' => $detials,
                'diagnosis_year' => $diagnosisYear . ' yrs',
                'examined_date' =>   $originalDiagnosisYear,
    ...
    {
        $IdData = PatientDtmsInformationMaster::select('smoking_question_id', 'alcohol_question_id')
            ->where('id', 1)->first();
    ...
    public function visitHistory(Request $request)
    {
        ...
        $prescriptionData = PatientPrescriptions::select('medicine_master.medicine_name', 'medicine_master.generic_name', 'tablet_type_master.tablet_type_name', 'patient_prescriptions.*')
     ...
     * @return mixed
     */
    public function prescriptionDocument($print_type = null)
    {
        $data = [];
