...
class MasterDataController extends Controller
{
    ...
    public function getSubDiagnosis()
    {
        ...
        $filldata = SubDiagnosisMaster::select('subdiagnosis_master.*', 'diagnosis_master.id as diagnosis_id', 'diagnosis_master.diagnosis_name')
            ->leftjoin('diagnosis_master', 'subdiagnosis_master.diagnosis_id', '=', 'diagnosis_master.id')
            ->where('subdiagnosis_master.is_deleted', 0)
            ->orderBy('subdiagnosis_master.id', 'DESC')
            ->get();
     ...
     * save subcomplication
     */
    public function saveSubDiagnosis(Request $request)
    {
        $validated = $request->validate([
            'diagnosis' => 'required',
            'sub_diagnosis_name' => 'required',
        ...
        if ($validated) {
            ...
            $ins = array(
                ...
                'subdiagnosis_name' => $request->sub_diagnosis_name,
                'code' => $request->sub_diagnosis_code,
                'branch_id' => Session::get('current_branch'),
                'display_status' => ($request->display_status == 'on') ? "1" : "0",
                'created_by' => Auth::id(),
                'is_deleted' => 0,
            ...
            if ($request->crude == 1) {
                $subdiagnosisCode = getAfeild("code", "subdiagnosis_master", [
                    ['code', $request->sub_diagnosis_code],
                ]);
                if ($subdiagnosisCode) {
                    return ['status' => 3, 'message' => "The code has already been taken"];
                    ...
                    $cond = array(
                        array(DB::raw('upper(subdiagnosis_name)'), strtoupper($request->sub_diagnosis_name)),
                        array('is_deleted', 0),

                    );

                    $getId = getAfeild("id", "subdiagnosis_master", $cond);

                    if ($getId) {
                        return ['status' => 4, 'message' => "Sub diagnosis already exist"];
                    } else {
                        DB::connection()->enableQueryLog(); // enable qry log
                        $insert_id = SubDiagnosisMaster::insertGetId($ins);
                        // TO GET LAST EXECUTED QRY
                        $query = DB::getQueryLog();
                        $lastQuery = end($query);
                        $sql = $lastQuery['query'];
                        ...
                        $log = array(
                            ...
                            'user_id' => Auth::id(), // userId
                            'log_type' => 1, // Save
                            'table_name' => 'SubDiagnosisMaster',
                            'qury_log' => $sql,
                            'description' => 'DTMS Master Save Sub Diagnosis ',
                            'created_date' => date('Y-m-d H:i:s')
                        );
                ...
                $subdiagnosisCode = getAfeild("code", "subdiagnosis_master", [
                    ['code', $request->sub_diagnosis_code],
                    ['id', '!=', $request->hid_subdiagnosis_id],
                ]);
                if ($subdiagnosisCode) {
                    return ['status' => 3, 'message' => "The code has already been taken"];
                } else {
                    $cond = array(
                        array(DB::raw('upper(subdiagnosis_name)'), strtoupper($request->sub_diagnosis_name)),
                        array('is_deleted', 0),
                        array('id', '!=', $request->hid_subdiagnosis_id)
                    );

                    $getId = getAfeild("id", "subdiagnosis_master", $cond);

                    if ($getId) {
                        return ['status' => 4, 'message' => "Subdiagnosis already exist"];
                    } else {
                        DB::connection()->enableQueryLog(); // enable qry log
                        $insert_id = SubDiagnosisMaster::whereId($request->hid_subdiagnosis_id)->update($ins);
                        // TO GET LAST EXECUTED QRY
                        $query = DB::getQueryLog();
                        $lastQuery = end($query);
                        $sql = $lastQuery['query'];
                        ...
                        // LOG  // add parameters based on your need
                        $log = array(
                            'primarykeyvalue_Id' => $request->hid_subdiagnosis_id,
                            'user_id' => Auth::id(), // userId
                            'log_type' => 2, // Update
                            'table_name' => 'SubDiagnosisMaster',
                            'qury_log' => $sql,
                            'description' => 'DTMS Master, Update Sub Diagnosis ',
                            'created_date' => date('Y-m-d H:i:s')
                        );
     ...
     * delete subcomplication
     */
    public function deleteSubDiagnosis(Request $request)
    {
        $id = $request->id;
        if ($id) {
            $ins = array(
            ...
            DB::connection()->enableQueryLog(); // enable qry log
            $insert_id = SubDiagnosisMaster::whereId($id)->update($ins);
            // TO GET LAST EXECUTED QRY
            $query = DB::getQueryLog();
            $lastQuery = end($query);
            $sql = $lastQuery['query'];
            ...
            $log = array(
                ...
                'table_name' => 'SubDiagnosisMaster',
                'qury_log' => $sql,
                'description' => 'DTMS Master, Delete Sub Diagnosis ',
                'created_date' => date('Y-m-d H:i:s')
            ); 
    ...
    //jdc
    public function loaddiagnosisDropdown()
    {
        $filldata = DiagnosisMaster::where('is_deleted', 0)->where('display_status', 1)->orderByDesc('id')->get();
        $options = "<option value=''>Select</option>";
        if ($filldata) {

            foreach ($filldata as $value) {
                $options .= "<option value='$value->id'>$value->diagnosis_name</option>";
            }
        }
