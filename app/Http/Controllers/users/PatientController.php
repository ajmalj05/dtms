...
//                'is_main'=>1,
    ...
    public function searchPatient(Request $request)
    {
        ...
        // Check for the checkbox state and set conditions accordingly
        $exactSearch = !is_null($request->searchFilter) && $request->searchFilter === 'true'; //<!---01.11.24-->
        ...
        if (!is_null($request->uhid)) {
            $is_condition = 1;
            // Use exact match if the checkbox is checked
            $uhidCondition = $exactSearch ? ['uhidno', '=', $request->uhid] : ['uhidno', 'ILIKE', "%{$request->uhid}%"]; //<!---01.11.24-->
            array_push($cond, $uhidCondition);
        }
        ...
        // Fetch data
        if ($is_condition == 1) {
            $data = PatientRegistration::where($cond)->orderBy('id', 'DESC')->get();
        } else {
            $data = PatientRegistration::where($cond)->orderBy('id', 'DESC')->skip(0)->take(30)->get();
        }
