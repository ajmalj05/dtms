                                            ...
                                            <div class="card card-sm">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class=" mb-5">
                                                        ...
                                                        <div class="form-group ">
                                                            <label>Sub Diagnosis</label>
                                                            <select class="form-control dt-input SubDiagnosisSelect" id="SubDiagnosisSelect" name="subdiagnosis_id" disabled data-none-selected-text="Choose Sub Diagnosis">>
                                                            </select>
                                                            <small id="subdiagnosis_id_error" class="form-text text-muted error"></small>
                                                        </div>
                                                        <div class="form-group">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#SubDiagnosisSelect").on("change",function() {
            $("#subdiagnosis_id_error").html("");
        });
...
<script>
   var App = {
    initialize: function() {
        $('#diagnosis_name').on('change', App.showSubdiagnosisList);
    },
    showSubdiagnosisList: function() {
        var diagnosis_id = $('#diagnosis_name').val();
        $.ajax({
            url: "{{ route('get-subdiagnosis-list') }}",
            type: 'GET',
            data: {
            ...
            {
                $("#SubDiagnosisSelect").html('<option value="" selected>Choose a Subdiagnosis</option>');
                $.each(response.data, function(key, value) {
                    $("#SubDiagnosisSelect").append('<option value="' + value.id + '">' + value.subdiagnosis_name + '</option>');
                });
                $("#SubDiagnosisSelect").attr('disabled', false);
                    $("#SubDiagnosisSelect").selectpicker('refresh');;
            },
