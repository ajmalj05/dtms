...
<div class="common">
    ...
    <div style="text-align: center">
        <h2><u>Patient Education Programme</u></h2>
    </div>
    ...
    <div id="qansarea">
        @php
            $i =1;
        @endphp
        @foreach($data['pep_questions_data'] as $key => $questions)
            <b>Q.{{ $i }} :
            </b>{{$questions->question}} <br>
            <b>Ans. </b>
            @foreach($questions->sub_question as $label)
                @if($data['pep_history'])
                    @for($j=0;$j<count($data['pep_history']);$j++)
                        {{($data['pep_history'][$j]['answer']==$label->id) ? $label->label : "" }}
                    @endfor
                @endif
            @endforeach
            <br>
